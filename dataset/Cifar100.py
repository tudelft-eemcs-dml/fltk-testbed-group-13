...
class Cifar100Task():
    ...
    def getTaskDataSet(self):
        trainDataset = myCIFAR100(root=self.root, train=True, transform=self.transform_train, download=False, task_num=self.task_num)
        train_task_datasets = [CifarDataset(data, self.transform_train) for data in trainDataset.task_datasets] #the training samples partitioned into 10 tasks
        testDataset = myCIFAR100(root=self.root, train=False, transform=self.transform, download=False, task_num=self.task_num)
        test_task_datasets = [CifarDataset(data, self.transform) for data in testDataset.task_datasets] #the  test samples partitioned into 10 tasks
